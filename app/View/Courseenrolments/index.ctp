<div class="courseenrolments index">
	...
	<?php 
		...
		if ($courseenrolment['Courseenrolment']['member_id'] == $user['member_id']) {
	?>
	<tr>
		<td>
...
<div class="actions">
	...
	<ul>
		<li><?php echo $this->Html->link(__('Enrol in a Course'), array('action' => 'add')); ?></li>
		<?php
		    if($is_superuser | $is_officevolunteer) {
		?>
		<li><?php echo $this->Html->link(__('Course Conveyor View (Course Conveyor)'), array('action' => 'admin_index')); ?></li>
		<li><?php echo $this->Html->link(__('New Course Enrolment (Course Conveyor)'), array('action' => 'admin_add')); ?></li>
		<?php
		    }
		?>
	</ul>
